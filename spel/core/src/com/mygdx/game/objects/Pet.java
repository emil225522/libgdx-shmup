...
public class Pet {
	...
	public void update() {
		...
		this.idealPosition = new Vector2(Player.position.x + Player.texture.getWidth(),
				Player.position.y + Player.texture.getHeight());
		...
		if (bulletTimer > 100) {
			...
			bullets.add(
					new Bullet(1, new Vector2(position.x + texture.getWidth(), position.y + (texture.getHeight() / 2)),
							TextureManager.BULLET_TEXTURE, 0, new Vector2(5, 0), 0.5f));
		}
